...
// ============================================================================
// Agent registry — JSON fallback config
// ============================================================================

const MISSING_AGENTS_MESSAGE =
  "Plugin config must include a non-empty 'agents' array. Set $CLAUDE_ACP_DELEGATE_CONFIG to a JSON file path, or place the config at ~/.config/claude/acp-delegate.json or ~/.claude/acp-delegate.json. " +
  'Example: { "agents": [{ "id": "gemini", "command": ["gemini", "--acp"] }] }'

const CONFIG_ENV_VAR = "CLAUDE_ACP_DELEGATE_CONFIG"
...
function fallbackConfigPaths(): string[] {
  ...
  paths.push(join(homedir(), ".config", "claude", "acp-delegate.json"))
  paths.push(join(homedir(), ".claude", "acp-delegate.json"))
  return paths
}
...
function validateAgent(raw: unknown, index: number): AgentConfig {
  ...
  if (typeof candidate.id !== "string" || candidate.id.length === 0) {
    throw new Error(
      `Agent config at index ${index} is invalid: 'id' must be a non-empty string`,
    )
  }
  if (!Array.isArray(candidate.command) || candidate.command.length === 0) {
    throw new Error(
      `Agent config at index ${index} is invalid: 'command' must be a non-empty string array`,
    )
  }
  for (let j = 0; j < candidate.command.length; j++) {
    if (typeof candidate.command[j] !== "string") {
      throw new Error(
        `Agent config at index ${index} is invalid: command[${j}] must be a string`,
      )
    }
  }
  if (
    ...
    throw new Error(
      `Agent config at index ${index} is invalid: 'label' must be a string when provided`,
    )
  }
  if (
    ...
    throw new Error(
      `Agent config at index ${index} is invalid: 'description' must be a string when provided`,
    )
  }
  if (
    candidate.whenToUse !== undefined &&
    typeof candidate.whenToUse !== "string"
  ) {
    throw new Error(
      `Agent config at index ${index} is invalid: 'whenToUse' must be a string when provided`,
    )
  }
  if (candidate.modelFlag !== undefined) {
    if (
      typeof candidate.modelFlag !== "string" ||
      candidate.modelFlag.length === 0
    ) {
      throw new Error(
        `Agent config at index ${index} is invalid: 'modelFlag' must be a non-empty string when provided`,
      )
    }
  }
  if (candidate.models !== undefined) {
    if (!Array.isArray(candidate.models) || candidate.models.length === 0) {
      throw new Error(
        `Agent config at index ${index} is invalid: 'models' must be a non-empty string array when provided`,
      )
    }
    for (let j = 0; j < candidate.models.length; j++) {
      ...
      if (typeof m !== "string" || m.length === 0) {
        throw new Error(
          `Agent config at index ${index} is invalid: models[${j}] must be a non-empty string`,
        )
      }
    }
    ...
    for (const m of candidate.models) {
      if (seen.has(m)) {
        throw new Error(
          `Agent config at index ${index} is invalid: models contains duplicate '${m}'`,
        )
      }
      seen.add(m)
    }
  ...
  if (candidate.defaultModel !== undefined) {
    if (
      ...
      throw new Error(
        `Agent config at index ${index} is invalid: 'defaultModel' must be a non-empty string when provided`,
      )
    }
    if (
  ...
  if (
    candidate.autoApprove !== undefined &&
  ...
  return {
    ...
    ...(candidate.defaultModel !== undefined
      ...
      : {}),
    ...(candidate.modelFlag !== undefined
      ? { modelFlag: candidate.modelFlag }
      : {}),
  }
}
...
const STATE_DIR_ENV = "CLAUDE_ACP_DELEGATE_STATE_DIR"
...
function getStateDir(): string {
  ...
  if (xdg && xdg.length > 0) return join(xdg, "claude", "acp-delegate")
  return join(homedir(), ".local", "state", "claude", "acp-delegate")
}
...
function emptyState(): AcpState {
  return {
    version: STATE_FILE_VERSION as 1,
    updatedAt: Date.now(),
    pid: process.pid,
    inflight: [],
    recent: [],
...
function coerceStopReason(value: unknown): AcpStopReason | undefined {
  return typeof value === "string" &&
    VALID_STOP_REASONS.has(value as AcpStopReason)
...
async function readBoundedTextFile(
  ...
  const sliced = lines.slice(
    startLine - 1,
    limit !== undefined ? startLine - 1 + limit : undefined,
  )
  ...
  const sendRequest = (
    ...
    return new Promise((resolveReq, rejectReq) => {
      ...
      try {
        ...
        rejectReq(
          new AcpError(
            `Failed to write to agent stdin: ${(err as Error).message}`,
            "EIO",
          ),
        )
      }
        ...
        void (async () => {
          try {
            const result = await readBoundedTextFile(
              cwdAbs,
              (msg.params ?? {}) as ReadTextFileParams,
            )
        ...
        for (const kind of preferredKinds) {
          chosen = options.find(
            (o): o is PermissionOption =>
              !!o &&
              typeof o === "object" &&
              (o as PermissionOption).kind === kind &&
              typeof (o as PermissionOption).optionId === "string",
          )
          if (chosen) break
        }
        if (chosen) {
      ...
      try {
        await Promise.race([
          ...
          new Promise<void>((res) =>
            setTimeout(() => res(), SESSION_CLOSE_TIMEOUT_MS).unref(),
          ),
        ])
      } catch {}
    }
...
// ============================================================================
// Health probe — fired (not awaited) at server startup
// ============================================================================
...
async function probeAgent(agent: AgentConfig): Promise<HealthEntry> {
  ...
  let child: ChildProcessWithoutNullStreams
  try {
    child = spawn(binary, agent.command.slice(1), {
      cwd: process.cwd(),
      stdio: ["pipe", "pipe", "pipe"],
    })
  } catch (e) {
...
async function probeAll(agentRegistry: AgentConfig[]): Promise<HealthEntry[]> {
...
async function expandSinglePath(
  ...
  if (info.isDirectory()) {
    ...
    for (const entry of entries) {
      ...
      const childBlocks = await expandSinglePath(
        cwdAbs,
        `${rel}/${entry}`,
        budget,
      )
...
async function buildContextPreamble(
  ...
  for (const b of blocks) {
    if (b.status === "ok" && b.content !== undefined) {
      ...
      parts.push(
        `<context path="${b.path}"${trunc}>\n${b.content}\n</context>`,
      )
    } else {
      parts.push(
        `<context path="${b.path}" skipped="true" reason="${b.reason ?? "unknown"}"/>`,
      )
    }
  }
  return parts.join("\n") + "\n\n"
...
function summarizeAgent(agent: AgentConfig): string {
  if (agent.whenToUse !== undefined && agent.whenToUse.length > 0)
    return agent.whenToUse
  if (agent.description !== undefined && agent.description.length > 0) {
    const firstSentence =
      agent.description.split(/(?<=[.!?])\s+/, 1)[0] ?? agent.description
...
function buildRoutingBlock(agentRegistry: AgentConfig[]): string {
  const lines = agentRegistry.map((a) => {
...
function applyStopReasonTrailer(
  ...
  if (stopReason === "refusal") {
    ...
    return {
      output: `delegate_to_${agentId} refused: ${reason}`,
      status: "error",
    }
  }
  if (stopReason === "cancelled") {
    return {
      output: `delegate_to_${agentId} cancelled by agent.`,
      status: "cancelled",
    }
  }
...
async function runDelegation(
  ...
  ctx: { directory: string; abort?: AbortSignal },
): Promise<{ output: string; metadata: Record<string, unknown> }> {
  const startedAt = Date.now()
  const callId = randomUUID()
  const sessionIdShort = "" // MCP has no session ID in tool context
  ...
  try {
    const preamble =
      args.includeContext && args.includeContext.length > 0
        ? await buildContextPreamble(cwd, args.includeContext)
        : ""
...
function makeDelegateTool(agent: AgentConfig): RegisteredTool {
